				...
				public function payment_receipt()
				{
					...
					if (isset($payment_receipt_data) && count($payment_receipt_data) > 0) {
						...
						if($data['statutory_deductions_radio'] != "No" && !($pr_id > 0)){
							...
							$pr_added_data['it_ids_percentage'] = $data['it_ids_percentage'];
							$pr_added_data['it_ids_amount'] = $data['it_ids_amount'];
							$pr_added_data['gtds_percenatge'] = $data['gtds_percenatge'];
							$pr_added_data['gtds_amount'] = $data['gtds_amount'];
						}
					}
        	     ...
        	     public function debit_note($post_data = [])
	     {
			...
			public function save_voucher_data() {
					...
					if ($voucher_id == "") {
						$head = $this->input->post($head_field);
						$project_id = $this->input->post('project_id');
						$check_unique_data = $this->admin_model->check_unique_voucher_data($head,$project_id);
					
						if(count($check_unique_data) > 0){
							$success = 0;
				...
				public function save_voucher_transaction_data() {
					...
					if ($transaction_id == "") {
						...
						if(count($pending_voucher_data) > 0){
							$success = 0;
							$messages = "Transaction data is pending input from $head.";
