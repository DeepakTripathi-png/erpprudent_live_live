...
class PHPExcel_Worksheet_BaseDrawing implements PHPExcel_IComparable
{
    ...
    // Resize proportional?
    if ($this->_resizeProportional && $pValue != 0) {
        // Check if height or width are zero before performing the division
        if ($this->_width == 0 || $this->_height == 0) {
            // Handle the case when either dimension is zero
            // For example, set a default height or skip resizing
            $this->_height = $pValue; // or some default value based on your needs
        } else {
            $ratio = $this->_height / $this->_width;
            $this->_height = round($ratio * $pValue);
        }
