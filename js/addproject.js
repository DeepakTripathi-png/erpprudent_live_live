...
$(document).ready(function(){
  ...
  $("#test_report").click(function () {
    if ($(this).is(":checked")) {
      var html = '<hr style="margin: 15px 0;"><div class="row dvtest_report">'
      + '<div class="col-md-4">'
      + '<label class="">Test Report <span class="require" aria-required="true" style="color:#a94442">*</span></label> <span id="test_report_error" style="font-size:12px;color:#a94442;"></span>'
      + '</div>'
      + '<div class="col-md-8">'
      + '<div style="display:flex;">'
      + '<div class="form-group" style="width: 90%;">'
      + '<div class="input-icon right"><i class="fa"></i>'
      + '<input type="text" class="form-control" id="test_report_text1" name="test_report_text[]"  placeholder="Test Report" required>'
      + '</div>'
      + '</div><div class="" style="width: 10%;">'
      + '<div class="adtestreportbtn"><i class="fa fa-plus" aria-hidden="true" style="font-size:10px;"></i></div>'
      + '</div></div>'
      + '</div>'
      + '</div><div class="adtest_report" id="adtest_report"></div>';
      $('#didetlsa').append(html);
    } else {
      $(".dvtest_report").remove();
    }
  });

  var test_reportt = 2;
  $(document).on('click', '.adtestreportbtn', function () {
    var test_report_text = $("#test_report_text1").val();
    if (test_report_text != '' && typeof test_report_text != 'undefined') {
      var html = '<div class="row" id="pro' + test_reportt + '">'
      + '<div class="col-md-4">'
      + '<label class=""></label>'
      + '</div>'
      + '<div class="col-md-8">'
      ...
      + '<div class="form-group" style="width: 90%;">'
      + '<div class="input-icon right"><i class="fa"></i>'
      + '<input type="text" class="form-control" name="test_report_text[]" value="" placeholder="Test Report" required="">'
      + '</div>'
      + '</div><div class="" style="width: 10%;">'
      + '<div class="rmtestreportbtn" data-id="' + test_reportt + '"><i class="fa fa-minus" aria-hidden="true" style="font-size:10px;"></i></div>'
      + '</div></div>'
      + '</div>'
      + '</div>';
      $("#adtest_report").append(html);
      $("#test_report_error").html('');
      test_reportt++;
    } else {
      $("#test_report_error").html('Please enter test report value!');
    }
  });

  $(document).on('click', '.rmtestreportbtn', function () {
    var id = $(this).attr('data-id');
    $('#pro' + id).remove();
  });
