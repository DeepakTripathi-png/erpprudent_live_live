$( document ).ready(function() {
  ...
  var download_title = 'Vendor Performa Invoice';
  var recordsPerPage = 10;
    var ppi_value = $('#ppi').val();
  var ppi_count = $('#ppi_count').val();
  if ($('#ppi').val() != "") {
    ppi_value=  $('#ppi').val().trim()
  }
  if ($('#ppi_count').val() != "") {
    ppi_count  = $('#ppi_count').val().trim()
  }

   if (ppi_value == "") {
    $('.ppi_data').css("display", "none");
}
if (ppi_count == 0 && ppi_value != "") {
    $('.ppi_data').css("display", "none");
  ...
  var purchaseOrderlist = $('#purchaseOrderlist').DataTable({
    ...
    "oLanguage": {
      "sEmptyTable": "No Purchase Performa Invoice Found!"
    },
    "bDestroy" : true,
    "bInfo" : false,
    "ordering": true,
    ...
    "order": [],
    "ajax": {
      "url": base_url+"get_vendor_proforma_invoice_list?project_id="+$('#project_id').val()+"&type="+page_type,
      "type": "GET",
  ...
  $(document).on("click",".download-po",function(){
    ...
    $.ajax({
      url: base_url + 'get_vendor_pi_bom_detail',
      type: 'POST',
      dataType: 'json',
      data: {
      ...
      success: function(response) {
        ...
        var ws_data = [
          ["", "Purchase Order No.:-  "+ po_number,"Date",invoice_date,"","","","", "","","", "","","", ""],
          ["", "Purchase Proforma Invoice No. :- "+proforma_no, "","","","","","", "","","", "","","", ""],
          ["","", "","","", "","","", "","","", ""], // Header row
          ["","To", "","","", "","","", "","","", "","","", ""],
        ...
        worksheet.getCell(1, 4).font = boldFont;
        worksheet.getCell(1, 3).font = boldFont;
        worksheet.getCell(1, 2).font = boldFont;
        worksheet.getCell(2, 2).font = boldFont;
        ...
        // worksheet.getCell(14, 2).font = boldFont;
        ...
        const tnc_text = worksheet.getCell(tnc_row+1, 2); // or worksheet.getCell('B16')
        ...
        workbook.xlsx.writeBuffer().then(function(buffer) {
          ...
          saveAs(blob, 'vendor_performa_invoice.xlsx');
        });
  ...
  $(document).on("click",".editRecordVPI",function(){
    var vpi_trancsaction_id = $(this).attr("rel-vpi-id");
    $('.ppi_data').css("display", "");
    var project_id = $(this).attr("data-project-id");
    // alert(base_url+"get_edit_bom_vpi_item_by_project");
    if(project_id){
      ...
      var table = $('#Editbomindentitmdisplay').DataTable({
        ...
        "order": [],
        "ajax": {
          "url": base_url+"get_edit_bom_vpi_item_by_project",
          "type": "POST",
          "data":{ vpi_trancsaction_id: vpi_trancsaction_id,project_id:project_id },
          'dataSrc':function(result) {
            ...
            $("#ppi").val(result.po_number);
            $("#performa_id_edit").val(result.performa_invoice_id);
            $("#edit_terms_and_condition").val(result.terms_and_condition);
            $("#edit_remark").val(result.remark);
            $("#transaction_id_edit").val(result.transaction_id);
            $(".add_po_form").hide();
            $(".edit_po_form").show();
            var content = $('#edit_terms_and_condition').val();  // Get the textarea content
                var formattedContent = content.replace(/\n/g, '<br>');  // Replace newlines with <br>
            $('#edit_terms_and_condition_val').val(formattedContent);
            $(document).on('keyup','#edit_terms_and_condition',function(e){
                var content = $('#edit_terms_and_condition').val();  // Get the textarea content
                var formattedContent = content.replace(/\n/g, '<br>');  // Replace newlines with <br>
                $('#edit_terms_and_condition_val').val(formattedContent);
            });
  ...
  $('#purchaseOrderlist tbody').on('click','.openview', function () {
    ...
    if (row.child.isShown()) {
      ...
      if(transaction_type == 'bom_upload'){
        ...
        download_title = 'Vendor Perfoma Invoice';
      }
