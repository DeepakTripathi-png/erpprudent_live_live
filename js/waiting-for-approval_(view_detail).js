...
var download_title = 'Waiting for Approval';
...
var table = $('#boqtranslist').DataTable({
    ...
    "bAutoWidth": false,
    "oLanguage": {
        "sEmptyTable": "Waiting for Approval not Available!"
    },
    "bDestroy" : true,
    "ordering": true,
    "searching":true,
    ...
    "ajax": {
        ...
        "type": "POST",
        "data":{project_id:project_id,status_txt:status_txt}
    },
...
$(document).on("change",".filter", function(){
    ...
    download_title = 'Waiting for Approval';
    ...
    if(filter == 'original'){
        if(transaction_type =='boq_exceptional_appr'){
            download_title = 'Waiting for Approval (BOQ Exceptional Approval)';
        }else if(transaction_type == 'boq_upload' || transaction_type == 'add_edit_boq'){
            download_title = 'Waiting for Approval (BOQ Upload)';
        }
        ...
        var otable = $('#originalpboqview'+transaction_id).DataTable({
            ...
            "ajax": {
                ...
                "type": "POST",
                "data":{project_id:project_id,filter:filter,transaction_id:transaction_id,transaction_type:transaction_type,status_txt:status_txt}
            },
            "columnDefs": [{ 
                "targets": [0,3,4,5],
                "orderable": false
        ...
        if(transaction_type =='boq_exceptional_appr'){
            ...
            download_title = 'Waiting for Approval (BOQ Upload Without GST)';
        }
        ...
        var caltable = $('#calwithoutgstpboqview'+transaction_id).DataTable({
            ...
            "ajax": {
                "url": base_url+"project_boq_trans_list_display",
                "type": "POST",
                "data":{project_id:project_id,filter:filter,calculatedfiler:calculatedfiler,transaction_id:transaction_id,transaction_type:transaction_type,status_txt:status_txt}
            },
...
$(document).on("change",".calculatedfiler", function(){
    ...
    if(calculatedfiler == 'without_gst'){
        ...
        if(transaction_type =='boq_exceptional_appr'){
            ...
            download_title = 'Waiting for Approval (BOQ Upload With GST)';
        }
    ...
    $('#boqtranslist tbody').on('click','.openview', function () {
        ...
        $.ajax({
                ...
                'data': {filter:filter,transaction_id:transaction_id,transaction_type:transaction_type,project_id:project_id,status_txt:status_txt},
        ...
        if (row.child.isShown()) {
            ...
            if(transaction_type == 'boq_upload' || transaction_type == 'add_edit_boq' ||transaction_type == 'boq_exceptional_appr'){
                actionhtml = '<th scope="col" style="vertical-align: top;text-align:right;">Action</th>';
                actionhtmlfooter = '<th style="text-align:right;font-weight:600;"></th>';
            }
...
function generate_table(rowData) {
    ...
    var html = '<hr>';
    var html = '<div class="portlet-body" style="padding-top:0;" id="displayed' + number + '">'
    ...
    + '<th scope="col" style="vertical-align: top;">Unit</th>'
    + '<th scope="col" style="vertical-align: top;">EA Qty</th>'
    ...
    html += '</tbody>'+ '</table></div></div><hr style="margin: 5px 0;border:0;">';
    return html;
}
...
$(document).on('click','.active_link_wt_approve' , function(){
    var this1 = $(this);
    bootbox.confirm("Are you sure?", function(result) 
    {
        if(result)
        {
            ...
            $.ajax({
                ...
                {
                    const tableId=$(this1).closest('table').attr('id');
...
$('.nav-tabs a').on('shown.bs.tab', function (e) {
    ...
    if(tabIndex == 2) {
        initializeDataTable();
        // if ( $.fn.dataTable.isDataTable( '#bomtranslist' ) ) {
        //     table_bom = $('#bomtranslist').DataTable();
        // } else {
        //     table_bom = $('#bomtranslist').DataTable({
        //     "dom": 'Bfrtip',
        //     "lengthMenu": [
        ...
        //     // Load data from an Ajax source
        //     "ajax": {
        //         "url": base_url+"get_bom_transaction_list",
        //         "type": "POST",
        //         "data":{project_id:project_id,status_txt:status_txt}
        //     },
        //     //Set column definition initialisation properties
...
$('#bomtranslist tbody').on('click','.openview', function () {
...
$(document).on('change','.statusselectbom' , function(e){
    ...
    bootbox.confirm("Are you sure?", function(result) 
    {
        if(result)
        {
            ...
            var boq_code = this1.attr('data-boq_code');             
            ...
            $.ajax({
                ...
                type:'POST',
                dataType:'json',
                data:{id:id,status:status,project_id:project_id,boq_code:boq_code},
                success:function(data)
                {
                    // bootbox.alert(data.msg, function() {
                    //     setTimeout(function(){
...
// for get sub item of payment transaction
$(document).on("click",".opentransactionview",function(){
