...
$(document).on("change","#billing_split_up",function(){
	...
	if(proforma_final != ""){
		...
		if(auto_round_off && auto_round_off_val > 0){
		...
		if(gst_type == "cgst_sgst"){
			$("#proforma_final1").html(proforma_final);
		}else{
			$("#proforma_igst_final").html(proforma_final);
		}
		...
		$(".total_type_amount").html(proforma_final.toFixed(2));
	}else{
		proforma_final = proforma_final;
		$(".total_type_amount").html(proforma_final);
	}
	$("#project_billing_type_value").val(percentage_val);
...
// alert($("#project_billing_type").val());
// $('.billing_split_up').select2({
