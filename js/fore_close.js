$( document ).ready(function() {
  ...
  if (po_count == 0 && po_value != "") {
    ...
    var foreCloseList = $('#foreCloseList').DataTable({
      "scrollX": true,
      "dom": 'Bfrtip',
      "lengthMenu": [
    ...
    $(document).on('input', 'input[name="bom_fore_close[]"]', function() {
      var $input = $(this);
      var $row = $input.closest('tr');
      
      var foreCloseMax = parseFloat($input.data('max'));
      var currentValue = parseFloat($input.val());
     
      if (currentValue > foreCloseMax) {
          $input.val(foreCloseMax);  
      }
      ...
      var foreCloseValue = parseFloat($input.val());  
     
      var totalAmount = (rate * foreCloseValue) + (rate * foreCloseValue * gst / 100);
      // console.log(rate + "," + foreCloseValue + ", " + gst);
      // alert(totalAmount)
    ...
    $(document).on('input', ".js-po-basic_rate", function() {
      ...
      const qty = parseFloat($row.find('[name="bom_fore_close[]"]').val()) || 0;
  ...
  GST No- 27AAKCP4656L1Zs`;
   ...
   worksheet.getCell(1, 1).font = { size: 18 };
        ...
        var table = $('#Editbomindentitmdisplay').DataTable({
          ...
          "ajax": {
            ...
            'dataSrc':function(result) {
              ...
              $("#edit_vdc_number").val(result.vdc_number);
    ...
    $('#foreCloseList tbody').on('click','.openview', function () {
    ...
    $("#force_close_form").validate({
        ...
        messages: {
          vdc_number: {
            required: "Please Select VDC Number."
          },
            "bom_fore_close[]": {
      ...
      $('#vdc_number').change(function() {
        var vdcNumber = $(this).val(); 
        var po_number = $("#po_number").val();
        var project_id_val = $("#project_id_val").val();
        if (vdcNumber !== "") {
            $.ajax({
                url: 'get_fore_close_bom_item',  
                type: 'POST',                  
                data: {vdc_number: vdcNumber,po_number:po_number,project_id:project_id_val}, 
                success: function(response) {
                  $('#bomindentitmdisplay').removeClass('hide');
                  $('#bomindentitmdisplay tbody').html(response);
                },
