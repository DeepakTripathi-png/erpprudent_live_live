      ...
      var table = $('#edit_bomgrnitmdisplay').DataTable({
        ...
        "ajax": {
          ...
          'dataSrc':function(result) {
            ...
            console.log(result);
            var grn = result.grn;
            var taxDeductions = result.grn_tax_data;
            ...
            $("#edit_it_ids_percentage").val(grn.it_ids_percentage);
            $("#edit_it_ids_amount").val(grn.it_ids_amount);
            $("#edit_tcs_percenatge").val(grn.tcs_percenatge);
            $("#edit_tcs_amount").val(grn.tcs_amount);
            $("#edit_total_amount_with_deduction").val(grn.total_amount_with_deduction);
          ...
          var container = $(".tax-deduction-block");
          // container.empty(); // Clear existing content
            
          taxDeductions.forEach(function (item, index) {
              var taxDeductionRow = `
                  <div class="tax-deduction-row">
                      <div class="col-md-6">
                          <div class="form-group">
                              <label>Tax Deduction Description <span class="require" style="color:#a94442">*</span></label>
                              <input type="text" class="form-control" name="tax_deduction_description[]" value="${item.tax_deduction_description}" placeholder="Tax Deduction Description">
                          </div>
                      </div>
                      <div class="col-md-5">
                          <div class="form-group">
                              <label>Tax Deduction Amount <span class="require" style="color:#a94442">*</span></label>
                              <input type="text" class="form-control onlyNumericInput tax_deduction_amount edit_tax_deduction_amount" name="tax_deduction_amount[]" value="${item.tax_deduction_amount}" placeholder="Tax Deduction Amount">
                          </div>
                      </div>
                      <div class="col-md-1">
                          <div class="form-group">
                              <br> <button type="button" class="edit-remove-tax-deduction" data-index="${index}">Remove</button>
                          </div>
                      </div>
                  </div>`;
              
              container.append(taxDeductionRow);
          });
  ...
  $("#grn_form").validate({
    ...
    messages: {
      ...
      invoice_amount: {
        ...
        equalTo: "Invoice amount must be equal to the GRN Amount."
      },
      invoice_number: {
...
//     // Validate all existing tax deduction descriptions and amounts
//     $('.tax-deduction-row').each(function() {
//         var description = $(this).find("input[name='tax_deduction_description[]']").val().trim();
//         var amount = $(this).find("input[name='tax_deduction_amount[]']").val().trim();

//         if (description == "" || amount == "") {
//             isValid = false;
//             // alert("Please fill in all Tax Deduction Description and Amount fields before adding a new one.");
//             $(".alert-container").removeClass("hide");
        
//             $(".alert-container").html('<div class="alert modify alert-danger">Please fill in all Tax Deduction Description and Amount fields before adding a new one.</div>');
//             setTimeout(function() {
//                  $(".alert-container").addClass("hide");
//               $(".alert-container").html('');
...
//             return false; // Break loop on first invalid input
//         }
//     });
...
//     // Append new row only if all fields are filled
//     var newElement = `
...
$(document).on('click', '.add-tax-deduction', function() {
  ...
  var data_name_cls = ""; // Default value

  if (dataName === 'edit-tax-data') {
      data_name_cls = "edit";
  } else if (dataName === 'add-tax-data') {
      data_name_cls = "add";
  }
  ...
  <div class="tax-deduction-row extra-row">
      ...
      <div class="col-md-5">
          <div class="form-group">
              ...
              <input type="text" class="form-control onlyNumericInput tax_deduction_amount ${data_name_cls}_tax_deduction_amount" name="tax_deduction_amount[]" value="" placeholder="Tax Deduction Amount">
          </div>
      </div>
      ...
      <div class="col-md-1">
          <div class="form-group">
              <br>
              <button type="button" class="${data_name_cls}-remove-tax-deduction">Remove</button>
          </div>
      </div>
  </div>
  `;
  ...
  // Call the appropriate function based on the data-name
  if (dataName == 'edit-tax-data') {
    console.log("edit-ok");
      total_amount_with_deduction_edit();
  } else if (dataName == 'add-tax-data') {
    console.log("ok");
    
      total_amount_with_deduction_add();
  }
});



$(document).on('click', '.edit-remove-tax-deduction', function() {
  $(this).parents(".tax-deduction-row").remove();
  total_amount_with_deduction_edit();
});

$(document).on('click', '.add-remove-tax-deduction', function() {
  $(this).parents(".tax-deduction-row").remove();
  total_amount_with_deduction_add();
});

$(document).on('input', '.edit_tax_deduction_amount', function() {

  total_amount_with_deduction_edit();
});

$(document).on('input', '.add_tax_deduction_amount', function() {

  total_amount_with_deduction_add();
});

$("#it_ids_percentage").change(function () {
  var percentage = parseFloat($(this).val()); // Get selected percentage
  var dcAmount = parseFloat($(".dc_amount_add").val()); // Get dc_amount value
  console.log(percentage);
  console.log(dcAmount);
  
  if (!isNaN(percentage) && !isNaN(dcAmount)) {
      var calculatedAmount = (dcAmount * percentage) / 100; // Calculate percentage
      $("#it_ids_amount").val(calculatedAmount.toFixed(2)); // Set value with 2 decimal places
  } else {
      $("#it_ids_amount").val(""); // Reset if invalid
  }
  total_amount_with_deduction_add()
});

$("#tcs_percenatge").change(function () {
  var percentage = parseFloat($(this).val()); // Get selected percentage
  var dcAmount = parseFloat($(".dc_amount_add").val()); // Get dc_amount value
  
  if (!isNaN(percentage) && !isNaN(dcAmount)) {
      var calculatedAmount = (dcAmount * percentage) / 100; // Calculate percentage
      $("#tcs_amount").val(calculatedAmount.toFixed(2)); // Set value with 2 decimal places
  } else {
      $("#tcs_amount").val(""); // Reset if invalid
  }
  total_amount_with_deduction_add()
});


$("#edit_it_ids_percentage").change(function () {
  var percentage = parseFloat($(this).val()); // Get selected percentage
  var dcAmount = parseFloat($(".dc_amount_edit").val()); // Get dc_amount value
  console.log(percentage);
  console.log(dcAmount);
...
$("#edit_tcs_percenatge").change(function () {
  var percentage = parseFloat($(this).val()); // Get selected percentage
  var dcAmount = parseFloat($(".dc_amount_edit").val()); // Get dc_amount value
...
// $(document).on("input", "input[name='tax_deduction_amount[]']", total_amount_with_deduction);
...
function total_amount_with_deduction_add() {
  var total = 0;
  var dcAmount = parseFloat($(".dc_amount_add").val()); 
  // Get IT TDS Amount
  var itTdsAmount = parseFloat($("#it_ids_amount").val()) || 0;

  // Get TCS Amount
  var tcsAmount = parseFloat($("#tcs_amount").val()) || 0;

  // Get Sum of All Tax Deduction Amounts
  var taxDeductionTotal = 0;
  $("input[name='tax_deduction_amount[]']").each(function () {
      var taxAmount = parseFloat($(this).val()) || 0;
      taxDeductionTotal += taxAmount;
  });

  // Calculate Total Amount with Deduction
  total = dcAmount - (itTdsAmount + tcsAmount + taxDeductionTotal);

  // Update Total Amount Field
  $("#total_amount_with_deduction").val(total.toFixed(2));
}


function total_amount_with_deduction_edit() {
  var total = 0;
  var dcAmount = parseFloat($(".dc_amount_edit").val()); 
  // Get IT TDS Amount
  var itTdsAmount = parseFloat($("#edit_it_ids_amount").val()) || 0;

  // Get TCS Amount
  var tcsAmount = parseFloat($("#edit_tcs_amount").val()) || 0;
  ...
  // Update Total Amount Field
  $("#edit_total_amount_with_deduction").val(total.toFixed(2));
}